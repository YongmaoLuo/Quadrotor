...
#define GYR_KP 2*0.5f // kp，ki用于控制加速度计修正陀螺仪积分姿态的速度
#define GYR_KI 2*0.05f
#define GYR_KD 0.0f
#define SAMPLE_RATE 500.0f // 每秒中解算次数
...
class imu {
    ...
    double before_acc_x[ACC_BEFORE_NUMBER]={0},before_acc_y[ACC_BEFORE_NUMBER]={0},before_acc_z[ACC_BEFORE_NUMBER]={0};//储存前十个加速度计的值
    double before_gyr_x[GYR_BEFORE_NUMBER]={0},before_gyr_y[GYR_BEFORE_NUMBER]={0},before_gyr_z[GYR_BEFORE_NUMBER]={0};
    ...
    double q0 = 0, q1 = 0, q2 = 0, q3 = 0; //表示整个旋转的四元数
  ...
  public:
    ...
    void Mahony_Filter_Init(double ax,double ay,double az);
    void Mahony_Filter_Update();

    void Traditional_Linear_Filter_Init(double ax,double ay,double az);
